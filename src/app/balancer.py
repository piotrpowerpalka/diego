...
class Balancer:
    ...
    def __init__(self, drow, limits, roles, forecast):
        if isinstance(drow, pd.DataFrame):
            self.date = pd.Timestamp(drow["Datetime"].values[0])
        ...
        self.during_peak = {
            period: False if (period < 24) or (period > 56) else True
            for period in range(96)
        }
        self.num_ep = 8  # number_of_export_periods
        self.bess_should_export = {
            period: True if 32 <= period <= 72 else False for period in range(96)
        }
        ...
        self.pred.loc[:, "ms_observ_Eq"] = self.pred.loc[:, ["ms_Eq", "pv_Eq"]].sum(
            axis=1
        )
        self.initial_state = self.pred.copy()
        self.avail_pv_energy = (
            self.initial_state["ms_Ep"].values[0] + self.pred["pv_Ep"].values[0]
        )
    ...
    def calc_old_autocons(self):
        if self.initial_state["ms_Ep"].values[0] != 0.000:
            self.old_pv_Ep_auto = -(
                self.initial_state["pv_Ep"] / self.initial_state["ms_Ep"]
            ).values[0]
        else:
        ...
        if self.initial_state["ms_Eq"].values[0] != 0.000:
            self.old_pv_Eq_auto = -(
                self.initial_state["pv_Eq"] / self.initial_state["ms_Eq"]
            ).values[0]
        else:
            self.old_pv_Eq_auto = nan

    def get_full_state(self):
        if self.date < pd.Timestamp(2024, 6, 24, 0, 15):
            max_capacity = 177.6
        else:
            max_capacity = 263
    ...
    def make_autocons_df(self):
        self.auto_cons = pd.DataFrame(
            data=[self.pv_Ep_auto_cons, self.pv_Eq_auto_cons],
            index=["pv_Ep_auto", "pv_Eq_auto"],
        ).T
    ...
    def calculate_tgs(self):
        if self.initial_state["ms_observ_Ep"].values[0] != 0:
            self.tg_old = (
                self.initial_state["ms_observ_Eq"] / self.initial_state["ms_observ_Ep"]
            ).values[0]
        else:
            self.tg_old = nan
    ...
    def scale_wp(self):
        ...
        self.work_points = pd.DataFrame(
            data=self.work_points.values(), index=list(self.work_points.keys())
        ).T
        self.wps_frame = pd.concat(
            [self.init_wp, self.work_points, self.work_points - self.init_wp]
        )
        self.wps_frame.index = ["old", "new", "diff"]

    def update_blocks(self):
        self.dev_Eq_blocked.update(
            {laser: False for laser in ["bystar1", "bysprint", "bystar2", "mazak"]}
        )
        ...
        try:
                ...
                for ind, offer in self.Ep_offers.iterrows():
                    ...
                    if offer["device"] == "pv":
                        if self.avail_pv_energy < 0:
                            ...
                            pv_to_bess_export = min(
                                abs(self.avail_pv_energy),
                                12.5,
                            ...
                            self.pred["soc"] = (
                                self.pred["soc_Ep"] / self.bess_full_state
                            ) * 100
                        ...
                        if self.bess_should_export[self.period]:
                            ...
                            contr_bess_energy = (
                                self.pred["soc_Ep"].values[0]
                                - self.bess_full_state * 0.2
                            )
                            ...
                            if self.pred["pv_Ep"].values[0] >= 0:
                                ...
                                self.pred["soc_Ep"] += min(
                                    bess_import,
                                    self.bess_full_state - bess_import,
                                    self.bess_full_state
                                    - self.pred["soc_Ep"].values[0],
                                )
                        ...
                        if not self.dev_Eq_blocked[offer["device"]]:
                            ...
                            if (self.pred[offer["device"] + "_Ep"].values[0] > 0) and (
                                self.avail_pv_energy <= -1 * dev_energy_max
                            ):

                                self.work_points[offer["device"] + "_P"] = dev_power_max
                                self.dev_Energies[offer["device"] + "_Ep"] = (
                                    dev_energy_max
                                )
                    ...
                    if Eqk >= 0:
                        for ind, offer in self.Eq_offers.iterrows():
                            if offer["device"] == "pv":
                                ...
                                Emax = 50 * 0.25
                                Ep_bess = (
                                    self.pred["soc_Ep"] - self.initial_state["soc_Ep"]
                                ).values[0]
                                ...
                                if (Emax**2 - Ep_bess**2) < 0:
                                    print(
                                        (
                                            self.date,
                                            self.pred["soc_Ep"].values[0],
                                            self.initial_state["soc_Ep"].values[0],
                                        )
                                    )
                                else:
                                ...
                                if Eqk > 0:
                                    ...
                                    if (
                                        self.pred[offer["device"] + "_Eq"].values[0] > 0
                                    ) and (self.unc_dev_Eq_oper[offer["device"]]):
            ...
            if Eqk > 0:
                ...
                sort_lasers = (
                    ...
                    .reset_index(drop=True)
                    .sort_values(by=0, axis=1, ascending=True)
                )
            ...
            for col in delta_cols:
                self.energy_deltas[col] = (
                    self.pred[col] - self.initial_state[col]
                ).values[0]
            ...
            for energy in ["Ep", "Eq"]:
                ...
                self.Ems_new_obs[energy] = (
                    self.Ems_new[energy] + self.pred["pv_" + energy]
                )

                self.pred["ms_observ_" + energy] = self.Ems_new_obs[energy]

                self.energy_deltas[energy] = (
                    self.Ems_new_obs[energy] - self.pred["ms_observ_" + energy]
                ).values[0]

            if self.Ems_new["Ep"] != 0:
                self.pv_Ep_auto_cons = (
                    -self.pred["pv_Ep"].values[0] / self.Ems_new["Ep"]
                )
            else:
                self.pv_Ep_auto_cons = nan

            if self.Ems_new["Eq"] != 0:
                self.pv_Eq_auto_cons = (
                    -self.pred["pv_Eq"].values[0] / self.Ems_new["Eq"]
                )
            else:
